...
import { PrefectureSelectSm } from '../src/components/block';
...
// 都道府県チェックボックス(画面サイズsm以下UI時)に関するテスト
describe('Left Bar Item Component', () => {
	...
	// 都道府県チェックボックスの数が都道府県データ47個分かどうか
	test('renders 47 prefecture items correctly', async () => {
		render(<TestWrapper />);

		// prefecturesが表示されるまで待機
		const prefectureElements = screen.getAllByTestId(/checkbox-/);
	...
	test('selects the correct prefecture when clicked', async () => {
		...
		// 最初の都道府県ボタンを取得
		const firstPrefectureButton = screen.getByTestId('checkbox-1'); // 北海道のテストID
